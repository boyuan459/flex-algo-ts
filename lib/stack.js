"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.longestValidParentheses = exports.isValidParenthesisV2 = exports.isValidParentheses = exports.minRemoveToMakeValid = void 0;
var MapParentheses = {
    '(': ')',
    '{': '}',
    '[': ']',
};
function minRemoveToMakeValid(s) {
    var chars = s.split('');
    var stack = [];
    for (var i = 0; i < chars.length; i++) {
        if (chars[i] === '(') {
            stack.push(i);
        }
        else if (chars[i] === ')') {
            if (stack.length === 0) {
                chars[i] = '';
            }
            else {
                stack.pop();
            }
        }
    }
    for (var j = 0; j < stack.length; j++) {
        chars[stack[j]] = '';
    }
    return chars.join('');
}
exports.minRemoveToMakeValid = minRemoveToMakeValid;
function isValidParentheses(s) {
    var stack = [];
    for (var i = 0; i < s.length; i++) {
        var parenthese = s[i];
        if (MapParentheses[parenthese] !== undefined) {
            stack.push(parenthese);
        }
        else {
            var left = stack.pop();
            if (left === undefined) {
                return false;
            }
            if (MapParentheses[left] !== parenthese) {
                return false;
            }
        }
    }
    return stack.length === 0;
}
exports.isValidParentheses = isValidParentheses;
function isValidParenthesisV2(s) {
    var sLeftParenthesis = [];
    var sAsterisk = [];
    for (var i = 0; i < s.length; i++) {
        if (s[i] === '(') {
            sLeftParenthesis.push(i);
        }
        else if (s[i] === ')') {
            if (sLeftParenthesis.length) {
                sLeftParenthesis.pop();
            }
            else if (sAsterisk.length) {
                sAsterisk.pop();
            }
            else {
                return false;
            }
        }
        else {
            sAsterisk.push(i);
        }
    }
    while (sLeftParenthesis.length && sAsterisk.length) {
        var pos = sLeftParenthesis.pop();
        var apos = sAsterisk.pop();
        if (apos < pos) {
            return false;
        }
    }
    return sLeftParenthesis.length === 0;
}
exports.isValidParenthesisV2 = isValidParenthesisV2;
function longestValidParentheses(s) {
    var max = 0;
    var left = 0;
    var right = 0;
    for (var i = 0; i < s.length; i++) {
        if (s[i] === '(') {
            left += 1;
        }
        else {
            right += 1;
        }
        if (left === right) {
            max = Math.max(max, left + right);
        }
        if (right > left) {
            left = right = 0;
        }
    }
    left = right = 0;
    for (var i = s.length - 1; i >= 0; i--) {
        if (s[i] === '(') {
            left += 1;
        }
        else {
            right += 1;
        }
        if (left === right) {
            max = Math.max(max, left + right);
        }
        if (left > right) {
            left = right = 0;
        }
    }
    return max;
}
exports.longestValidParentheses = longestValidParentheses;
